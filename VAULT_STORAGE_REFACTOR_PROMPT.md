...
### 3. Cleanup Old Format (No Migration)

Since the app hasn't been released yet, we'll simply clear old SharedPreferences data if detected:
...
Future<void> _cleanupOldStorageFormat() async {
  ...
  // App hasn't been released, so we can safely drop old format data
  Log.info('Detected old storage format - clearing old data');
  await prefs.remove('encrypted_vaults');
  Log.info('Cleared old SharedPreferences vault storage');
}
```

#### Call cleanup in `initialize()`:
- Check if old SharedPreferences key exists
- If yes, remove it (clear old data)
- Then proceed with normal initialization (empty vault list)
...
1. **Old format cleanup**: Test that old SharedPreferences data is cleared on first run
...
6. **Fresh start**: Verify app works correctly after old data is cleared
...
- **Old format detected**: Clear old SharedPreferences data, start fresh
...
- ✅ Old SharedPreferences format is detected and cleared
- ✅ App starts fresh after format change (no migration needed)
...
- This is a breaking change for storage format
- Since app hasn't been released, we can safely drop old format data
- Old SharedPreferences data will be cleared on first run after this change
- File system storage is more appropriate for this use case than SharedPreferences
- Users will need to recreate vaults after this change (acceptable for pre-release)
