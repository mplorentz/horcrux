...
void main() {
  test('drift_schemas/ contains the dump matching the current schema', () async {
    ...
    final committed = File('drift_schemas/drift_schema_v6.json');
    expect(committed.existsSync(), isTrue,
        reason: 'drift_schemas/drift_schema_v6.json is missing. Run '
            '`dart run drift_dev schema dump lib/database/app_database.dart drift_schemas/`.');
    ...
    try {
      ...
      final fresh = File('${tempDir.path}/drift_schema_v6.json');
      expect(fresh.existsSync(), isTrue, reason: 'drift_dev did not emit drift_schema_v6.json');
      ...
      expect(
        ...
        reason: 'drift schema in code does not match drift_schemas/drift_schema_v6.json. '
