...
void main() {
  ...
  group('processInvitationInvalid', () {
    const ownerPubkey = TestHexPubkeys.alice;
    const vaultId = 'test-invalid-invite-vault';
    ...
    setUp(() {
      ...
      mockNdkService = MockNdkService();
      when(mockNdkService.getCurrentPubkey()).thenAnswer((_) async => ownerPubkey);
      mockNotificationService = MockHorcruxNotificationService();
      mockPushReceiver = MockPushNotificationReceiver();
      mockRelayScanService = MockRelayScanService();
    ...
    Nip01Event makeInvalidEvent({String? vaultIdTag, String? reason}) {
      final tags = <List<String>>[
        ['vault_id', vaultIdTag ?? vaultId],
        ['reason', reason ?? 'This invitation has already been redeemed'],
      ];
      return Nip01Event(
    ...
    test('archives the vault with the reason from tags', () async {
      await addTestVaultWithShare();
      final event = makeInvalidEvent(reason: 'Already redeemed by another user');
      await service.processInvitationInvalid(event: event);

      final vault = await repository.getVault(vaultId);
      expect(vault, isNotNull);
      expect(vault!.isArchived, isTrue);
      expect(vault.archivedReason, 'Already redeemed by another user');
    });
    ...
    test('throws ArgumentError when vault_id tag is missing', () async {
      ...
      expect(
        () => service.processInvitationInvalid(event: event),
        throwsA(isA<ArgumentError>()),
      );
    });
    ...
    test('no-ops when vault not found (already deleted)', () async {
      final event = makeInvalidEvent(vaultIdTag: 'nonexistent-vault');
      await service.processInvitationInvalid(event: event);
    });
  });
