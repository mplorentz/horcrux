...
class DistributionDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<DistributionRow>> forVault(String vaultId) =>
      (select(distributions)
            ..where((d) => d.vaultId.equals(vaultId))
            ..orderBy([(d) => OrderingTerm.desc(d.version)]))
          .get();

  Future<DistributionRow?> latestForVault(String vaultId) =>
      (select(distributions)
            ..where((d) => d.vaultId.equals(vaultId))
            ..orderBy([(d) => OrderingTerm.desc(d.version)])
            ..limit(1))
  ...
  Future<void> insertDistribution(DistributionsCompanion row) =>
      into(distributions).insert(row);

  Future<List<DistributionShareRow>> sharesFor(String distributionId) =>
      (select(distributionShares)
            ..where((s) => s.distributionId.equals(distributionId)))
          .get();

  Future<void> insertShare(DistributionSharesCompanion row) =>
      into(distributionShares).insert(row);
